...
class ClassEncoder(nn.Module):
    def __init__(self, config, use_embedding=True):
        ...
        if config['y_vocab'] ** config['y_len'] > 16 ** 4:
            self.sampling_type = 'simple'
            print('Warning: y_vocab ** y_len > 16 ** 4, using simple sampling instead of unique sampling.')
        else:
            self.sampling_type = 'unique'
    ...
    def sample_codebook(self, batch_size, device):
        ...
        if self.sampling_type == 'simple':
            return torch.randint(
                self.config['y_vocab'],
                [batch_size, self.config['tasks'], self.config['y_len']],
                device=device)
        else:
            cls_int = [
                np.random.choice(
